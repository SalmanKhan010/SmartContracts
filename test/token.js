...
describe("Token Contract", function(){
    ...
    describe("Transactions", function(){
        ...
        if("Should fail if sender does not have enough tokens", async function(){
            const initialOwnerBalance = await hardhatToken.balanceOf(owner.address);4
            await expect(hardhatToken.connect(addr1).transfer(owner.address,5));
            const addr2Balance = await hardhatToken.balanceOf(addr2.address);
            expect(addr2Balance).to.equal(5);
        });
...
describe("Token contract", function(){
    ...
    it("Should transfer token between accounts" ,async function(){
        ...
        const hardhatToken = await Token.deploy(); //deployment of Contract
        //Trasnfer 10 tokens from owner to addr1
